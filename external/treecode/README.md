...
formula to advance the positions `**r**` and velocities
`**v**` from step `n` to step `n+1` is:
```
...
`math.h` by prepending the letter `r' (for
`real`).  The following functions are used by **treecode**;
sources appear in [`mathfns.c`](https://legacy.ifa.hawaii.edu/faculty/barnes/treecode/mathfns.c.html).
